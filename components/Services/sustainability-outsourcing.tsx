...
//       .select("*")
//       .eq("title", "Sustainability Outsourcing");
//     if (error) {
...
const sanitizeHTML = (html: string) => { // {{ edit_2 }}
    return {
...
const SustainabilityOutsourcing = () => {
  ...
  const [serviceProviderData, setProviderData] = useState<{
    title: string;
    heading: string; // Fixed type
    content: string; // Fixed type
    significanceBgImage: string;
    significance: string;
    planOfAction: string;
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-20">
          <div className="flex flex-wrap">
            ...
            <div className="w-full pl-4 lg:w-2/3">
              ...
              <p className="mb-4" dangerouslySetInnerHTML={sanitizeHTML(serviceProviderData?.content ?? '')}></p> {/* {{ edit_3 }} */}
